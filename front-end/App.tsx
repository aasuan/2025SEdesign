  ...
  return (
    <Layout user={user} onLogout={() => setUser(null)} currentView={currentView} onChangeView={setCurrentView}>
      ...
      {currentView === 'exam-manager' && (
        <ExamManager exams={availableExams} questionBank={questions} onUpdateExam={handleUpdateExam} onDeleteExam={handleDeleteExam} />
      )}
      {currentView === 'create-exam' && (
        <CreateExam questionBank={questions} onSave={handleSaveExam} onCancel={() => setCurrentView('teacher-dashboard')} />
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {availableExams.map(exam => (
              <div
                key={exam.id}
                className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm hover:border-indigo-300 transition-colors group"
              >
      ...
      {currentView === 'student-profile' && <Profile />}
    </Layout>
  );
};
